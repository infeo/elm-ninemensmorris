...
-- returns if the selected nodes are ok
...
validTurns curr pass dummy fstNod sndNod = 
    let st = wrapper' curr
        ...
        relOk = if st == slideState || st == jumpState then fstNod /= sndNod
                else
                if st == hasMillState || st == putState then fstNod == sndNod
                else True
...
stepPlayerS oldId newId curr pass dummy =
    let st = wrapper curr 
        ...
        help = pass.numOfStones
    in  if st == putState
        then (newCurr, pass, {dummy | myFields = List.filter (\x -> x /= newId) dummy.myFields })
        else
        if st == slideState
        then (newCurr, pass, {dummy | myFields = oldId :: (List.filter (\x -> x /= newId) dummy.myFields)})
        else
        if st == jumpState
        then (newCurr, pass, {dummy | myFields = oldId:: (List.filter (\x -> x /= newId) dummy.myFields)})                
        else
        ...
        if st == checkState
        then let var = (not << List.isEmpty) <| checkMovement newCurr dummy 
                 pl = {curr | canMove = var}
                 newPl = trans pl wizard
             in ( {newPl | playing = False}, {pass | playing = True}, dummy)  
        else (newCurr, pass, dummy)
...
-- setzt voraus, dass sich bei jedem stepForward der Zustand ändert
